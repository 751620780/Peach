    ...
    class Program
    {
        ...
        private static void GenerateXMLFile(string xmlFilePath)
        {
            try
            {
                ...
                XmlElement rootElement = myXmlDoc.CreateElement("Computers");//创建xml的根节点

                myXmlDoc.AppendChild(rootElement);//将根节点加入到xml文件中（AppendChild）


                XmlElement firstLevelElement1 = myXmlDoc.CreateElement("Computer");//初始化第一层的第一个子节点

                firstLevelElement1.SetAttribute("ID", "11111111");//填充第一层的第一个子节点的属性值（SetAttribute）
                firstLevelElement1.SetAttribute("Description", "Made in China");

                rootElement.AppendChild(firstLevelElement1);//将第一层的第一个子节点加入到根节点下

                XmlElement secondLevelElement11 = myXmlDoc.CreateElement("name");//初始化第二层的第一个子节点

                secondLevelElement11.InnerText = "Lenovo";//填充第二层的第一个子节点的值（InnerText）
                ...
                myXmlDoc.Save(xmlFilePath);//将xml文件保存到指定的路径下
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.ToString());
