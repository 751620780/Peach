...
namespace PeachValidator
{
    public partial class MainForm : Form
    {
        ...
        private void toolStripButtonOpenPit_Click(object sender, EventArgs e)
        {
            ...
            pitFileName = ofd.FileName;//获得PIT文件的完整路径
            setTitle();

            Regex re = new Regex("##\\w+##");
            if (File.Exists(pitFileName) && re.IsMatch(File.ReadAllText(pitFileName)))//PIT文件存在且该文件中存在与re正则表达式匹配的项
            {
                //表明该pit文件中还包含了一个含有Defines的xml文件，也需要载入
                ofd.Title = "Select PIT defines file";

                if (ofd.ShowDialog() != System.Windows.Forms.DialogResult.OK)
                    return;

                var defs = PitParser.parseDefines(ofd.FileName);//解析该文件

                foreach (var kv in defs)//把解析出来的字典，加入到该字典中
                {
            ...
            parserArgs[PitParser.DEFINED_VALUES] = DefinedValues;//把上面得到的字典放入到解析参数的字典
